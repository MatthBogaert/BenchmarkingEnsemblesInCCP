...
```
Friedman test with Rom's posthoc procedure: compare all against the best ranked. 
...
#Against the best (rom)
test <- postHocTest(ranks_auc_test, 
                    test="friedman",
                    control = 'GSA',
                    correct="rom", 
                    use.rank=FALSE)
plotRanking(pvalues=test$corrected.pval, summary=test$summary, alpha=0.05)
#plotPvalues(test$corrected.pval, show.pvalue = FALSE)

#Write the results to excel
#data.frame(rank = round(t(test$summary),3),
#           pvalues = round(t(test$corrected.pval),3)) %>% 
#  write.csv2(file = './aucs_comparison_stat.csv')
...
colMeans(ranks_acc_test)
colMeans(ranks_acc_test) %>% which.min()

#Against the best (rom)
test <- postHocTest(ranks_acc_test, 
                    test="friedman",
                    control = 'nnbl',
                    correct="rom", 
                    use.rank=FALSE)
...
#data.frame(rank = round(t(test$summary),3),
 #          pvalues = round(t(test$corrected.pval),3)) %>% 
#  write.csv2(file = './accs_comparison_stat.csv')
...
colMeans(ranks_tdl_test)
colMeans(ranks_tdl_test) %>% which.min()

#Against the best (rom)
test <- postHocTest(ranks_tdl_test, 
                    test="friedman",
                    control = 'STACK',
                    correct="rom", 
                    use.rank=FALSE)
...
#data.frame(rank = round(t(test$summary),3),
#           pvalues = round(t(test$corrected.pval),3)) %>% 
#  write.csv2(file = './tdls_comparison_stat.csv')
...
colMeans(ranks_empc_test)
colMeans(ranks_empc_test) %>% which.min()

#Against the best (rom)
test <- postHocTest(ranks_empc_test, test="friedman",control = 'GSA',
                    correct="rom", 
                    use.rank=FALSE)
plotRanking(pvalues=test$corrected.pval, summary=test$summary, alpha=0.05)
#plotPvalues(test$corrected.pval, show.pvalue = FALSE)
...
#data.frame(rank = round(t(test$summary),3),
 #          pvalues = round(t(test$corrected.pval),3)) %>% 
  #write.csv2(file = './empcs_comparison_stat.csv')
...
all_ranks %>% 
  rowwise() %>% 
  mutate(Average = mean(c_across()), .keep = 'unused') %>% 
  bind_cols(Algo = classifier_names) %>% 
  select(Algo, Average) 
# %>% write.csv2(file = './average_ranks_stat.csv')
